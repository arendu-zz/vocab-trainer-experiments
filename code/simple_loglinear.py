...
class SimpleLoglinear(object):
    def __init__(self, dh, regularization = 0.1, learner_regularization=0.5):
        self.dh = dh #DataHelper(event2feats_file, feat2id_file, actions_file)
        self.l = regularization #regularization parameter
        self.ul = learner_regularization #regularization parameter for the learner model
        ...
        x[0] = 0.9
        x[1] = 0.1
        self.W_lr = theano.shared(floatX(x), name='W_lr')
        self.params = [self.W_lr]
        self.reg_params = [self.W_lr]
        self.make_graph()
    ...
    def make_graph(self):
        ...
        def recurrence(x_t, y_t, o_t, s_t, theta_t):
            ...
            #reg = 2 * theta_t  #TODO:add to gradient..
            if s_t[6] == 1: #nofeeback no knowledge change...
                theta_tp1 = theta_t
            else:
                p = T.nnet.sigmoid(self.W_lr)
                theta_tp1 = (p[0] * theta_t) + (p[1] * grad_theta_t)
            return theta_tp1, y_hat, loss_t

        [seq_thetas, seq_y_hats, seq_losses], _ = theano.scan(fn=recurrence, sequences=[X,Y,O,S], outputs_info=[theta_0, None, None])
        ...
        self.get_seq_losses = theano.function([X, Y, O, S, theta_0], outputs = seq_losses)
        self.get_params = theano.function(inputs = [], outputs = self.params)
        self.get_seq_loss = theano.function([X, Y, O, S, theta_0], outputs = seq_loss)
        self.get_total_loss = theano.function([X, Y, O, S, theta_0], outputs = total_loss)
        ...
        self.do_sgd_update = theano.function([X, Y, O, S, theta_0, lr], outputs= [seq_loss, seq_thetas, seq_y_hats], updates = rmsprop(total_loss, self.params, lr))
        self.do_rms_update = theano.function([X, Y, O, S, theta_0, lr], outputs= [seq_loss, seq_thetas, seq_y_hats], updates = sgd(total_loss, self.params, lr))
